...
async function register(server, options) {
  ...
  options.serializers.req = stdSerializers.wrapRequestSerializer(
    options.serializers.req || stdSerializers.req
  );
  ...
  const validTags =
    Object.keys(tagToLevels).filter(key => levels.indexOf(tagToLevels[key]) < 0)
      .length === 0;
